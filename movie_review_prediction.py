...
# Поиск 3 отзывов наиболее схожих с заданным отзывом
norms = np.sum(weights_0_1 * weights_0_1, axis=1)
norms.resize(norms.shape[0], 1)
normed_weights = weights_0_1 * norms

# Проверяем наличие слов в индексе
# Если есть извлекаем их индекс
# Берем средне арифметическое по указанной оси
def make_sent_vect(words):
    indices = list(map(lambda x:word2index[x], filter(lambda x:x in word2index, words)))
    return np.mean(normed_weights[indices], axis=0)

# Вычисляем средний вектор для каждого обзора
...
def most_similar_reviews(review):
    ...
    # Те вектора которые оказались наиболее большими при перемножении те и являются наиболее подходящими по смыслу
